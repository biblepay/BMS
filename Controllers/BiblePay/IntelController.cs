    ...
    public class IntelController : Controller
    {
        ...
        public async Task<JsonResult> ProcessDoCallback([FromBody] ClientToServer o)
        {
            ...
            else if (o.Action == "admin_addexpense")
            {
                ...
                List<SponsoredOrphan> dt = DB.GetDatabaseObjectsAsAdmin<SponsoredOrphan>("sponsoredorphan");
                ...
                for (int i = 0; i < dt.Count; i++)
                {
                    ...
                    double nOldBalance = BMS.Report.GetChildBalance(oe.ChildID);
                    oe.Balance = nOldBalance += nAmount;
                    oe.Notes = sNotes;
                    oe.id = Guid.NewGuid().ToString();
                    bool f2 = DB.StoreDataAsAdmin<OrphanExpense>("options.orphanexpense", oe, oe.id);
                }
                if (sOpType == "PAYMENT")
                {
                    ...
                    bool f3 = DB.StoreDataAsAdmin<Expense>("options.expense", x, x.id);
                    ...
                    bool f4 = DB.StoreDataAsAdmin<Revenue>("options.revenue", r, r.id);
                }
            ...
            else if (o.Action == "scratch_submit")
            {
                ...
                {
                    BBPAPI.GovernanceProposal.SetScratch(sScratchID, sScratchValue);
                }
