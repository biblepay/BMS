...
{
     var res = o.replace(/\D/g, "");
     return res;
}

// handle an incoming call
// BIBLEPAY - RING HANDLER
const handleIncomingCall = ({ call }) => {
  ...
  setUpCall({ currentCall, isIncoming: true, number: call.number() });
    ...
    var e = {};
    e.number = call.number();
    console.log(e);

    DoCallback('Phone_Ring', e, '../../phone/processdocallback');

    // Convert PSTN to BBP inbound Narrative
    document.getElementById('caller-number').innerText = NumbersOnly(call.number());
...
const createCall = () => {
  cleanData();
  const numberInput = GetOutboundNumber();//document.getElementById('input-number');
  ...
  const myPhoneOutput = document.querySelector('.phoneoutput');
    myPhoneOutput.innerHTML = 'Dialing...'; // call state narr
