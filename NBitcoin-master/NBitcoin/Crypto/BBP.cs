...
namespace NBitcoin.Crypto
{
	...
	public class BBPTransaction
	{
		...
		public static Money EstFee(double virtualSize)
		{
			long n270000 = 270000;
			long n1000 = 1000;
			Money nFee = new NBitcoin.Money((long)(n270000 * (virtualSize / n1000)));
			nFee = nFee * 100;
			return nFee;
		}
		...
		public static string GetPubKeyFromPrivKey(bool fTestNet, string sPrivKey)
		{
			...
			{
                //Common.Log("Unable to extract pub key from priv key (getpubkeyfromprivkey)");
				return "";
			}
		}
			...
			string sSpendPrivKey, string sPayload, string sUTXOData, out string Error, out string TXID)
		{
			...
			try
			{
				...
				int iPos = 0;
				NBitcoin.Money nVinTotal = Money.Zero;
				foreach (var u in lu)
				{
				...
				NBitcoin.Money nSpent = Money.Zero;
				...
				NBitcoin.Money fees1 = EstFee(tx.GetVirtualSize() + sPayload.Length + (sPayload.Length * 25)) + new NBitcoin.Money((decimal)1.25, NBitcoin.MoneyUnit.BTC) * 2;
				txBuilder.SendFees(fees1);
				tx = txBuilder.BuildTransaction(true, sPayload);

				int nChangevOut = 0;
				Money nChangeAmt = Money.Zero;
				...
				for (int i = 0; i < lu.Count; i++)
				{
					...
					else
					{
						...
						u.SpentToNewChangeAmount = Money.Zero;
						BBPShared.ListSpentUTXO.Add(u);
						uUsed++;
