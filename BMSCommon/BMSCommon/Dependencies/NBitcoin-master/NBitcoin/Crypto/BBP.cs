...
namespace NBitcoin.Crypto
{
	...
	public static class BBPShared
	{
		...
		public static void Log(string sData, bool fQuiet = false)
		{
			try
			{
				string sPath = GetFolderLog("biblepaydll_nbitcoin.log");
	...
	public class BBPTransaction
	{
		private static void UpdateUTXOWithSpent(ref UTXO myUtxo, List<UTXO> lUTXO)
		{
			// Pointers to spent coins
			for (int i = 0; i < BBPShared.ListSpentUTXO.Count; i++)
			{
				UTXO SpentUTXO = BBPShared.ListSpentUTXO[i];
				if (SpentUTXO.TXID.ToString() == myUtxo.TXID.ToString())
				{
					...
					if (!bAlreadyUsed)
					{
						myUtxo.TXID = new NBitcoin.uint256(SpentUTXO.SpentToTXID);
						myUtxo.Amount = SpentUTXO.SpentToNewChangeAmount;
						myUtxo.index = SpentUTXO.SpentToIndex;
					}
				}
			}
		}

		public static List<UTXO> GetBBPUTXOs(bool fTestNet, string sAddress, string sData)
		{
			List<UTXO> l = new List<UTXO>();
			l = GetSpecifiedUTXO(fTestNet, sAddress, new Money(999999999999999999), sData);
			return l;
		}

		static int nRoundRobin = 0;
		...
		public static bool ValidateAddress(bool fTestNet, string sAddress)
		{
			...
			try
			{
				Network n = fTestNet ? NBitcoin.Network.TestNet : NBitcoin.Network.Main;

				NBitcoin.BitcoinAddress addr = NBitcoin.BitcoinAddress.Create(sAddress, n);
		...
		public static List<UTXO> GetSpecifiedUTXO(bool fTestNet, string sAddress, NBitcoin.Money nTotalAmount, string sData)
		{
			List<UTXO> lUTXO = new List<UTXO>();
			bool fValid = ValidateAddress(fTestNet, sAddress);

			if (!fValid)
			{
				BBPShared.Log("GetSpecifiedUtxo::InvalidAddress::" + sAddress + ", testnet=" + fTestNet.ToString());
				return lUTXO;
			}
			if (sAddress == null)
			{
			...
			//string sData = DownloadSanctuary(fTestNet, "/rest/getaddressutxos/" + sAddress);
			...
			while (true)
			{
				int nMyPointer = r.Next(0, lAllUTXO.Count);
				if (nMyPointer <= lAllUTXO.Count)
				{
					if (!lAllUTXO[nMyPointer].Chosen)
					{
						UTXO p = lAllUTXO[nMyPointer];
						p.Chosen = true;
						lAllUTXO[nMyPointer] = p;
						nConsumed++;

						UpdateUTXOWithSpent(ref p, lUTXO);
						if (p.Amount > 0)
						{
							lUTXO.Add(p);
							nTotalAdded += p.Amount;
							if (nTotalAdded > nTotalAmount)
								return lUTXO;
						}
					}
					if (nConsumed >= lAllUTXO.Count)
						break;
				}
			}
			return lUTXO;
		...
		/*
        public static DACResult PushTransactionRetired(bool fTestNet, string tx)
        {
            DACResult r = new DACResult();
            try
            {
                string sURL = Getanctuary(fTestNet) + "/rest/pushtx/";
            ...
            catch (WebException exception)
            {
                ...
                using (var reader = new System.IO.StreamReader(exception.Response.GetResponseStream(), encoding))
                {
                    string responseText = reader.ReadToEnd();
                    BBPShared.Log("Push Transaction::Error::" + responseText);
                    r.Error = responseText;
                    return r;
                }
            }
            catch (Exception ex)
            {
                r.Error = "Unable to push " + ex.Message;
                BBPShared.Log("PushTransaction::ERROR" + ex.Message);
            }
            return r;
        }
*/
		...
		public static double QueryAddressBalance(bool fTestNet, string sAddress, string sData)
		{
			try
			{
				List<UTXO> lu = GetSpecifiedUTXO(fTestNet, sAddress, new NBitcoin.Money(99999999, MoneyUnit.BTC), sData);
				double nAmount = 0;
				for (int i = 0; i < lu.Count; i++)
				{
					nAmount += (double)lu[i].Amount.ToDecimal(MoneyUnit.BTC);
				}
				return nAmount;
			}
			catch (Exception ex)
		...
		/*
        public static string CreateFundingTransaction2(bool fTestNet, double nAmount, string sSpendToAddress, string sSpendPrivKey, string sPayload, bool fSendForReal)
        {
            string sError = "";
            string sTXID = "";
            bool fResult = CreateFundingTransaction(fTestNet, nAmount, sSpendToAddress, sSpendPrivKey, sPayload, fSendForReal, out sError, out sTXID);
            return sTXID;
        }*/
		...
		public static string GetPubKeyFromPrivKey(bool fTestNet, string sPrivKey)
		{
			...
			string sPubKey = scSpendingKey.ScriptPubKey.GetDestinationAddress(n).ToString();
			return sPubKey;
		}


		public static bool PrepareFundingTransaction(bool fTestNet, double nAmount, string sSpendToAddress,
			string sSpendPrivKey, string sPayload, string sUTXOData, out string Error, out string TXID)
		{
			//DACResult r = new DACResult();
			Error = "";
			TXID = "";
			string sBurnAddress = GetBurnAddress(fTestNet);
			if (sSpendToAddress == "")
			{
			...
			try
			{
				...
				//NBitcoin.BitcoinSecret scSpendingKey = new NBitcoin.BitcoinSecret(sSpendPrivKey, fTestNet ? Network.BiblepayTest : Network.BiblepayMain);
				Network n =  fTestNet ? Network.TestNet: Network.Main;
				NBitcoin.BitcoinSecret scSpendingKey = new NBitcoin.BitcoinSecret(sSpendPrivKey, n);
				string sPubKeyOfPriv = scSpendingKey.ScriptPubKey.GetDestinationAddress(n).ToString();
				string sSpendingPublicKey = GetPubKeyFromPrivKey(fTestNet, sSpendPrivKey);

				List<UTXO> lu = GetSpecifiedUTXO(fTestNet, sPubKeyOfPriv, nEstTotal, sUTXOData);
				if (lu.Count == 0)
				{
				...
				var pkBurnAddress = new BitcoinPubKeyAddress(sBurnAddress, n);
				var pkSpendToAddress = new BitcoinPubKeyAddress(sSpendToAddress, n);
				...
				var txBuilder = new NBitcoin.TransactionBuilder(n);
				txBuilder.AddCoins(sourceCoins);
				txBuilder.AddKeys(scSpendingKey.PrivateKey);
				// This is the hardcoded max length in biblepaycore per vout
				int MAX_ML = 3000;
				double nReq = Math.Ceiling((double)(sPayload.Length / MAX_ML));
				nReq += 1;
				if (pkSpendToAddress.ToString() == "BB2BwSbDCqCqNsfc7FgWFJn4sRgnUt4tsM")
				{
					//nReq += 2;
				}
				...
				var tx = txBuilder.BuildTransaction(true, sPayload);
				...
				if (!fFullySigned)
				{
					Error = "Unable to fully sign.";
					return false;
				}
				string sPreTXID = tx.GetHash().ToString();

				TXID = txout;

				// At this point we must mark this pack of utxos as spent, and mark the spent to amount into the new utxo.
				int uUsed = 0;
				for (int i = 0; i < lu.Count; i++)
				{
					if (uUsed == 0)
					{
						UTXO u = lu[i];
						u.SpentToTXID = new NBitcoin.uint256(sPreTXID);
						u.SpentToIndex = nChangevOut;
						u.SpentToNewChangeAmount = nChangeAmt;
						BBPShared.ListSpentUTXO.Add(u);
						uUsed++;
						BBPShared.Log("adding as spent tx " + u.Amount.ToString());
					...
					else
					{
						...
						BBPShared.ListSpentUTXO.Add(u);
						uUsed++;
						BBPShared.Log("adding as spent tx 2.0 " + u.Amount.ToString());
	...
	public class BibleHash
	{
		...
		public void GetBookStartEnd(string sBook, ref int iStart, ref int iEnd)
		{
			sBook = sBook.ToUpper();
			iStart = 0;
			iEnd = 0;
		...
		private string cleanse(string w)
		{
			w = w.Trim();
			w = w.Replace("~", "");
			w = w.Replace("?", "");
		...
		public string GetRandomWords(int iWordCount)
		{
			List<string> l = GetWordList(7);
			string sData = "";
			Random r = new Random();
			for (int i = 0; i < iWordCount; i++)
			{

				string sWord = l[r.Next(0, l.Count - 1)];
				sData += sWord + " ";
			}
		...
		public List<string> GetWordList(int iMinLength)
		{
			List<string> l = new List<string>();
			for (int i = 0; i < kjv.b.Length; i++)
			{
				string sVerse = GetArrayElement(kjv.b[i], "|", 3);
				string[] vVerse = sVerse.Split(new string[] { " " }, StringSplitOptions.None);
				for (int j = 0; j < vVerse.Length; j++)
				{
					string w = cleanse(vVerse[j]);
					if (w.Length >= iMinLength)
					{
						if (!l.Contains(w))
							l.Add(w);
						if (l.Count > 10000)
		...
		public string GetVerse(string sBook, int iChapter, int iVerse, int iBookStart, int iBookEnd)
		{
			sBook = sBook.ToUpper();
		...
		public string GetSin(int iSinNumber, out string out_Description)
		{
		...
		public string GetBook(int iBookNumber)
		{
